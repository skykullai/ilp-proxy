...
async function fetchRenderedHtml(permitId) {
  ...
  try {
    ...
    // Extract all img srcs directly from the live DOM (absolute URLs)
...
// This is the key fix: we use Puppeteer to fetch the image so the request
// carries the same session/cookies as the page load, bypassing hotlink protection.
async function fetchImageAsBase64(imageUrl) {
  const page = await browser.newPage();
  try {
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36');

    // First, visit the referer page so cookies/session are set
    await page.goto('https://ilp.mizoram.gov.in/', {
      waitUntil: 'domcontentloaded',
      timeout: 15000,
    });

    // Now fetch the image URL via page.evaluate using fetch() inside the browser context
    // This means the request goes out with the correct Origin, Referer, and cookies
    ...
    return result; // { base64, contentType }
  } finally {
    await page.close();
  }
}
...
function pickPhoto(imgSrcs) {
  ...
  // First priority: exact pattern from the govt site
  ...
  // Second priority: any uploaded image file
...
app.get('/ilp/:permitId', async (req, res) => {
  ...
  try {
    ...
    // Embed the photo as a base64 data URI so Flutter can display it
    // without needing to send any special headers
    let photoUrl = null;
    if (rawPhotoUrl) {
      try {
        ...
        console.warn('Photo fetch failed, returning raw URL as fallback:', imgErr.message);
        // Fall back to the raw URL — Flutter will try with headers
...
// ── /photo — proxy a govt image URL through the server ────────────────────
// Usage: GET /photo?url=https://ilp.mizoram.gov.in/storage/...
// Flutter calls this instead of hitting the govt URL directly.
...
// ── /imgdebug/:permitId — lists all img URLs found on the page ────────────
...
// ── /debug/:permitId — raw rendered HTML ──────────────────────────────────
